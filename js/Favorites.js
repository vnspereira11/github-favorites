...
export class Favorites {
  ...
  createEmptyState() {
    ...
    const emptyFav = document.createElement("tr");
  ...
  tableScroll() {
    const tableFilled = this.root.querySelector(".table-container");
    
    if (this.entries.length > 3) {
    tableFilled.classList.add("table-scroll");        
    };
  }
  ...
  tableNoScroll() {    
    ...
    if (this.entries.length <= 3) {
      tableFilled.classList.remove("table-scroll");        
    };
  };
};
...
export class FavoritesView extends Favorites {
  ...
  onadd() {
    const addFavorite = this.root.querySelector(".add-favorite");

    window.document.onkeyup = event => {
      if(event.key === "Enter") {
        const { value } = this.root.querySelector("#user-search");
        this.add(value);      
        this.clearSearch(); 
      };
    };
  ...
  clearSearch() {
    let inputSearch = this.root.querySelector("#user-search");
    inputSearch.value = "";
  };
  ...
  update() {
    ...
    this.entries.forEach(user => {
      ...
      row.querySelector(".user img").alt = `Foto de perfil de ${user.name} no GitHub.`;
      row.querySelector(".user a").href = `https://github.com/${user.login}`;
      row.querySelector(".user p").textContent = user.name;
      row.querySelector(".user .user-login").textContent = user.login;
  ...
  createRow() {
    ...
    tr.innerHTML = `    
      <td class="user">
        <img src="https://github.com/vnspereira11.png" alt="Foto de perfil de Vanessa Pereira no GitHub.">
        <a href="https://github.com/vnspereira11" target="_blank">
          <p>Vanessa Pereira</p>          
          <span>&frasl;</span><span class="user-login">vnspereira11</span>
        </a>
      </td>
